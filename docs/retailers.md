...
- **Price ceiling `MAX_PRICE_CENTS` = €1.200 (2026-07-16).** A third-party reseller can also *win* the buy box when Amazon's own stock is gone — it still renders an add-to-cart button, so the add-to-cart signal alone counts it as in stock (observed live: a €867 reseller offer, plus "Neu & Gebraucht von 859,99 €", vs ~€750 retail). A featured offer priced **at/above the ceiling** is therefore reported `out_of_stock` (the observed price is kept for logging). A missing/unparseable price stays `in_stock` — a rare genuine restock must not become a false negative. NB: the buy box is transient (flips populated↔empty within minutes), so poll timing can still momentarily catch a below-ceiling reseller offer.
