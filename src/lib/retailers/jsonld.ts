...
const IN_STOCK = ["InStock", "LimitedAvailability", "OnlineOnly", "PreSale"];
const OUT_OF_STOCK = ["OutOfStock", "SoldOut", "Discontinued", "InStoreOnly"];

function mapAvailability(availability: string | undefined): StockStatus {
  if (!availability) return "unknown";
  const name = availability.split("/").pop() ?? "";
  if (IN_STOCK.includes(name)) return "in_stock";
  if (OUT_OF_STOCK.includes(name)) return "out_of_stock";
  return "unknown";
}
  ...
  for (const match of html.matchAll(scriptRe)) {
    ...
    while (stack.length) {
      ...
      if (obj["@type"] === "Product" && obj.offers) {
        ...
        const offer = (Array.isArray(offers) ? offers[0] : offers) as Record<
          string,
          unknown
        >;
